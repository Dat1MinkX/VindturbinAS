...
async function hent() {
    const table = document.getElementById("Sendt")
    const data = await fetch("/hent-Sendt")
    ...
    users.forEach(user => {
        ...
        b.innerText = user.Soknads-Tekst
        ...
        editBtn.addEventListener("click", () => {
            ...
            document.querySelector('input[name="soknad-stekst"]').value = user.Soknads-Tekst;
            document.querySelector('#role').value = user.Rolle || 'Bruker';
            document.querySelector('#originalUsername').value = user.Soknads-Tekst;
